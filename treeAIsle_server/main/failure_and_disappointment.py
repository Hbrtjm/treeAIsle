...
# The model should be universal, however we cannot allow arbitrary code execution. No antivirus would work excellently with that

from typing import List

class Model():
    def __init__(self,model_type: str ='adam',layers_sizes: List[int] =[10,10,1],layers_functions: List[str] =['relu','relu','output'],metrics: List[str]=['mean_absolute_error'],loss_function: str = 'mean_squared_error',activation_functions: List[str] = ['relu','relu','output'],library: str ='tensorflow'):
    ...
    def train(self,trainingDataset, resultDataset, epochs, chainId, currentHash, checkpointDirectory, chain, learningRate, unitsPerLayer,activation_functions):
        if self.library == 'tensorflow': # Generally that's not the way, it's for demonstration purposes
            ...
            # Makes sure that if model is not in the list below, tensorflow will still try to find it
            ...
            if chain.headBlock.depth > 0:
              chekcpoint_path = f"{checkpointDirectory}{chain.chain_id},{chain.headBlock.lastBlock.hash_}"
              self.model.load_weights(chekcpoint_path)
            current_hash = chain.headBlock.hash_
            checkpoint_path = f"{checkpointDirectory}{chainId},{current_hash}" # {branch}"
...
class Contestant():
    ...
    def train_model(self,model,trainingDataset,resultDataset,epochs,chain_id,current_block, checkpoint_directory, chain, learning_rate, units_per_layer,activation_functions):
                               #       trainingDataset, resultDataset, epochs, chain_id, current_hash, checkpoint_directory, hypervaules
        self.modelValues = model.train(trainingDataset,resultDataset,epochs,chain_id,current_block, checkpoint_directory, chain, learning_rate, units_per_layer,activation_functions)
...
class Contest():
    ...
    def compare(self,chain):
        ...
        for contestant in self.contestantPointers:
          print(f"{contestant.name} {contestant.performance}")
    ...
    def add_contestant(self,contestant):
        ...
        print(contestant.__str__())

    def generate_random_batch_size(self):
      pass
    def generate_random_hyperparameters(self):
      from random import uniform, choice
      learning_rate = 10**uniform(-5, -0.5)  # Random learning rate between 1e-5 and 1e-1
      num_layers = randint(2, 5)  # Random number of layers
      units_per_layer = [randint(10, 100) for _ in range(num_layers)]  # Random number of units per layer
      units_per_layer.append(1)
      activation_function = choice(['relu','sigmoid'])  
      activation_functions = [ activation_function ] * num_layers
      activation_functions.append('output')
      return learning_rate, units_per_layer, activation_functions
    def contest(self,epochs,test_amount,chain):
        ...
        for contestant in self.contestantPointers:
            ...
            learning_rate, units_per_layer, activation_functions = self.generate_random_hyperparameters()
            contestant.train_model(model,trainingData,trainingResult,epochs, chain.chain_id, chain.headBlock.hash_ , checkpoint_directory, chain, learning_rate, units_per_layer, activation_functions)
        for contestant in self.contestantPointers:
            performance = 0
...
def main(*args,**kwargs):
    ...
    contestant_1 = Contestant("First_contestant")
    testContest.add_contestant(contestant_1)
    contestant_2 = Contestant("Second_contestant")
    testContest.add_contestant(contestant_2)
    contestant_3 = Contestant("Third_contestant")
    testContest.add_contestant(contestant_3)
